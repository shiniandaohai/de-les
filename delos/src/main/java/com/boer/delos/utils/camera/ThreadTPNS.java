...
//		mActivity = activity;
	...
	public void run() {
		...
		try{
	        ...
	        {	
	        	...
	        	httpMethod.setURI(new URI( httpUrl + "/apns/apns.php?cmd=hello"));
			        	...
			        	if(entity != null)
			        	{
			        		...
			        		if(responseBody.indexOf("hi")!=-1)
			        		{
			        			...
			        			if(type == 2){
			        				...
			        				mHttpGetTool( httpUrl + "/apns/apns.php?cmd=reg_client&token="+DatabaseManager.s_GCM_token+"&appid="+DatabaseManager.s_Package_name+"&dev=0"+"&udid="+udid+"&os=android"+"&lang="+DatabaseManager.get_language()+"&osver="+DatabaseManager.get_osver()+"&appver="+appvar+"&model="+DatabaseManager.get_model());
			        				mHttpPostsyncApiTool( httpUrl + "/apns/apns.php",mcontext,udid,DatabaseManager.s_GCM_token,DatabaseManager.s_Package_name,"android");
			        			}else if(type == 1){
			        				//tm = (TelephonyManager) mActivity.getSystemService(Context.TELEPHONY_SERVICE);
				        			
			        				if(mReg == 1)
				        				mHttpGetTool( httpUrl + "/apns/apns.php"+"?cmd=unreg_mapping"+"&appid="+DatabaseManager.s_Package_name+"&uid="+mUID+"&udid="+udid+"&os=android",mUID);

				        			if(mReg == 2)
				        				mHttpGetTool( httpUrl + "/apns/apns.php"+"?cmd=reg_mapping"+"&appid="+DatabaseManager.s_Package_name+"&uid="+mUID+"&udid="+udid+"&os=android");
	...
	private JSONObject getSyncApiObject(Context mContext,String udid,String token,String appid,String os)
	{
        ...
        try 
        {
			...
			Cursor cursor = db.query(DatabaseManager.TABLE_DEVICE, new String[] { "_id", "dev_uid"}, null, null, null, null, "_id LIMIT " + HomepageListeningActivity.CAMERA_MAX_LIMITS);
