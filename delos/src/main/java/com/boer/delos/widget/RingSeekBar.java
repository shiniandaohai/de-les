...
public class RingSeekBar extends View {
    ...
    private Paint mProgressPercentPaint = null;
    ...
    ITouchUpEvent touchUpEventListener;
    private boolean cantTouch;


    public RingSeekBar(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void initView() {
        ...
        TypedArray localTypedArray = mContext.obtainStyledAttributes(mAttrs, R.styleable.RingSeekBar);
        ...
        mThumbDrawable = localTypedArray.getDrawable(R.styleable.RingSeekBar_thumb);
        ...
        float progressWidth = localTypedArray.getDimension(R.styleable.RingSeekBar_progress_width, 5);
        int progressBackgroundColor = localTypedArray.getColor(R.styleable.RingSeekBar_progress_background, Color.GRAY);
        int progressFrontColor = localTypedArray.getColor(R.styleable.RingSeekBar_progress_front, Color.BLUE);
        mSeekBarMax = localTypedArray.getInteger(R.styleable.RingSeekBar_progress_max, 100);
        ...
        mIsShowProgressText = localTypedArray.getBoolean(R.styleable.RingSeekBar_show_progress_text, false);
        int progressTextStroke = (int) localTypedArray.getDimension(R.styleable.RingSeekBar_progress_text_stroke_width, 5);
        int progressTextColor = localTypedArray.getColor(R.styleable.RingSeekBar_progress_text_color, Color.GREEN);
        mProgressTextSize = (int) localTypedArray.getDimension(R.styleable.RingSeekBar_progress_text_size, 100);
        ...
        DensityUtil densityUtil = new DensityUtil(mContext);
        mProgressPercentPaint = new Paint();
        mProgressPercentPaint.setColor(progressTextColor);
        mProgressPercentPaint.setAntiAlias(true);
        mProgressPercentPaint.setStrokeWidth(4);
        mProgressPercentPaint.setTextSize(densityUtil.dip2px(14));
    ...
    private void drawProgressText(Canvas canvas) {
        if (mIsShowProgressText) {
            ...
            canvas.drawText("%", mSeekBarCenterX + (textWidth) / 2, mSeekBarCenterY
                    + mProgressTextSize / 2, mProgressPercentPaint);
        } else {
            String mCurrentProgress = "OFF";
            float textWidth = mProgressTextPaint.measureText(mCurrentProgress + "");
            canvas.drawText(mCurrentProgress, mSeekBarCenterX - (textWidth) / 2, mSeekBarCenterY
                    + mProgressTextSize / 2, mProgressTextPaint);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                touchUpEventListener.showProgress(mCurrentProgress + "");
                break;
        }
        return true;
    }
    ...
    public interface ITouchUpEvent {

        void showProgress(String progress);

    }

    public void setITouchUpEvent(ITouchUpEvent touchUpEventListener) {
        this.touchUpEventListener = touchUpEventListener;

    }


    public void cantTouch(boolean cantTouch) {
        this.cantTouch = cantTouch;
