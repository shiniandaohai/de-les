 ...
 * 消息中心PopupWindow筛选条件弹出界面，对RecyclerView进行赋值
 */
public class MessageAdapter extends RecyclerView.Adapter<MessageAdapter.ViewHolder> implements View.OnClickListener {
    ...
    public ViewHolder onCreateViewHolder(ViewGroup viewGroup, int i) {
        View view = mInflater.inflate(R.layout.item_message, viewGroup, false);
    ...
    public void onBindViewHolder(final ViewHolder viewHolder, final int i) {
        ...
        viewHolder.tvItem.setText(mDatas.get(i));
        //将数据保存在itemView的Tag中，以便点击时进行获取
        viewHolder.itemView.setTag(i);

        if (isClicks.get(i)) {
            viewHolder.tvItem.setTextColor(Color.parseColor("#00a0e9"));
        } else {
